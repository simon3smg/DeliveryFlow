...
import { Truck, Lock, Mail, User, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-4 transition-colors duration-300">
      <div className="bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-800 relative animate-in fade-in zoom-in duration-300">
        ...
        <div className="bg-slate-950 p-8 text-center relative overflow-hidden">
          ...
          <h1 className="text-2xl font-bold text-white relative z-10 tracking-tight">Create Account</h1>
        ...
        <div className="p-8">
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                    ...
                    className={`py-3 rounded-xl border font-bold text-sm transition-all relative overflow-hidden ${role === 'driver' ? 'bg-indigo-900/20 border-indigo-500 text-indigo-400 ring-1 ring-indigo-500' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                >
                    Driver
                </button>
                <button
                    ...
                    className={`py-3 rounded-xl border font-bold text-sm transition-all relative overflow-hidden ${role === 'admin' ? 'bg-indigo-900/20 border-indigo-500 text-indigo-400 ring-1 ring-indigo-500' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}
                >
                    Admin
                </button>
              </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/40 transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 mt-4 ${loading ? 'opacity-80 cursor-not-allowed' : ''}`}
            >
               {loading ? <Loader2 className="animate-spin" size={20}/> : null} Create Account <ArrowRight size={20} />
            </button>
          </form>
